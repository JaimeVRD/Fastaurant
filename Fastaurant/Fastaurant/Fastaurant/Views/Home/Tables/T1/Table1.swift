...
struct Table1 : View {
    ...
    var body: some View{
        NavigationView{
            ...
            VStack{
                ...
                Text("Total: $ \(totalT1(), specifier: "%.2f")")
                    .font(.title3)
                    .fontWeight(.heavy)
                    .bold()
                                ...
                                EditTable1(drink: drink)){
                                    HStack{
                                        VStack(alignment: .leading, spacing: 6){
                                            ...
                                            Text("# Sold: \(Int(drink.soldOne))")
                                            
                                            Text("Total: $\(drink.soldOne * drink.cash, specifier: "%.2f")")
                                                .font(.body)
                                                .fontWeight(.medium)
                                                .foregroundColor(
                                                    drink.soldOne == 0 ? Color(.gray) : Color(.green))
    ...
    @discardableResult func toSummary1() -> Double{
        
        let valueSum1 : Double = 0
        
        for item in drink{
            
            item.soldOneT =  item.soldOneT + item.soldOne
            item.totalOne = item.soldOneT * item.cash
            item.soldOne = 0
    ...
    private func totalT1() -> Double{
        
        var valueTotal: Double = 0
        
        for item in drink{
            valueTotal += item.soldOne * item.cash
        }
        return valueTotal
    }
    
    @discardableResult func resetValue1() -> Double{
...
struct Table1Test_Previews: PreviewProvider {
